...
def copy_all(db, host, user, pw, port, docker_data_dir, pruned_f):
    ...
    for item in pruned_f:
        query = f'''
        CREATE TEMP TABLE tmp_table ON COMMIT DROP AS SELECT id, sNR, mode, mhz, rxTime, senderdxcc, flowstartseconds, senderCallsign, senderLocator, receiverCallsign, receiverLocator, receiverAntennaInformation, senderDXCCADIF, submode, insert_date FROM pskreporter_raw; \
        COPY tmp_table \
